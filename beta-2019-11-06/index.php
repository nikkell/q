...
*********************[WARNING]*******************
-------------------[ВНИМАНИЕ!]-------------------
------[СИНТАКСИС ДАННОГО КОДА МОЖЕТ НАНЕСТИ]-----
----[ПСИХОЛОГИЧЕСКИЕ ТРАВМЫ И ОСТАНОВКУ СЕРДЦА]--
-----[У ПРОГРАММИСТОВ НЕ ИСПОЛЬЗУЮЩИХ КОСТЫЛИ]---
---[ЕСЛИ ВЫ НЕ ИСПОЛЬЗУЕТЕ КОСТЫЛИ, БЫДЛОКОД, ]--
-[ГОВНОКОД, КОПИПАСТ ИЛИ ЛЕГКО ВПЕЧАТЛИТЕЛЬНЫ,]--
[ЗАКРОЙТЕ ЭТОТ ФАЙЛ, ТУТ ДОФИГА КОСТЫЛЕЙ И БРЕДА]
---------------[ВЫ ПРЕДУПРЕЖДЕНЫ]----------------
*********************[WARNING]*******************
*/?>
<!--<?php // * ?>Ох, братан, не советую я тебе смотреть этот код, пожалей глаза (vk.com/lodiur)-->
<?php
	require_once 'db.php'; // * файл для подключания базы данных и RedBeanPHP
...
<div class="double-scroll">
	...
	<div id="oh_blya">
		<script>
			...
			function rerere() {
				var reques = getXmlHttp();
				reques.open('GET', 'modal.php', false); 
				reques.send(null);	
				document.getElementById('oh_blya').innerHTML = " ";
				document.getElementById('oh_blya').innerHTML = reques.responseText;
			}
			setInterval(rerere, 10000);
		</script>
	</div>
</div>

<div class="chat-table-switcher">
	<header class="main-container fade relative-element"> <?php /* * блок с таблицей рейтинга пользователей */ ?>
		<div class="table-header header0tabla"> <?php /* * описание столбцов таблицы */ ?>
		...
		<div class="table-carousel">
			...
			<div id="nahuya">
			<!--ЕСЛИ ТЫ ЧИТАЕШЬ ЭТО СООБЩЕНИЕ ЗНАЧИТ КТО-ТО УЖЕ МЁРТВ-->
			<?php
...
function lol() {
	var req = getXmlHttp();
	req.open('GET', 'tablica.php', false); 
	req.send(null);	
	document.getElementById('nahuya').innerHTML = " ";
	document.getElementById('nahuya').innerHTML = req.responseText;
...
<?php endif; ?>
		...
		<a class="fade nno" style="display:none" id="prevCarousel">&#10094;</a> <?php /* * кнопка прокручивания карусели влево  */ ?>
		<a class="fade nno" style="display:none" id="nextCarousel">&#10095;</a> <?php /* * кнопка прокручивания карусели вправо  */ ?>
	</header>
	<div class="main-container chat-block relative-element fade">
		<?php /* Контейнер для всех колонок (переключатель чата, чат старый, чат новый, доп.секция) */ ?>
		<div class="flex-container">
			<?php /* Переключатель чатов */ ?>
			<div class="flex-line flex-border flex_line_1">
				<?php /* Список для чатов */ ?>
				<ul class="chat-list">
					<?php /*
...
<main class="accordion-container"> <?php /* * блок после таблицы */ ?>
	<div class="accordion-tab accordion-tab-ofset"> <?php /* * блок с главным лого, меню пользователя и ссылкой на группу VK */ ?>
		<input type="radio" name="accordion" id='main' checked />
		<label for="main">
			<article class="accordion-main">
				<div class="accordeon_target"></div> <?php /* * блок с главным лого */ ?>
				<content class="social-icons"> <?php /* * блок с меню пользователя и ссылкой на группу VK */ ?>
					<?php if ( isset ($_SESSION['logged_user']) ) : ?> <?php /* * для авторизованного пользователя, ссылка на открытие меню пользователя */ ?>
						<a href="#" class="social-account social-account-okay" id="showAccount" ;>
							<img width='35px' height='35px' src="svg/account.svg" alt="Аккаунт" />
						</a>
					<?php else : ?>
						<a class="social-account" href="#popup1"> <?php /* * для неавторизованного пользователя, ссылка на открытия окна авторизации/регистрации */ ?>
							<img width='35px' height='35px' src="svg/account.svg" alt="Аккаунт" />
						</a>
					<?php endif; ?>
					<a class='social-vk' id="chat_table_switcher"> <?php /* * ссылка на группу VK */ ?>
...
<div id="popup1" class="overlay overlay2">
	<div class="popup"> <?php /* * окно авторизации/регистрации пользователя */ ?>
		<a class="close close-reg" href="#">&times;</a> <?php /* * кнопка с закрытием окна */ ?>
		<div id="poopReglock" class="content">
			<form action="index.php" method="POST" id="modalform">
				<?php /* Регистрация */ ?>
				<?php /* * надпись, высвечивающаяся при ошибки в отправки формы регистрации */ ?>
				<p class="reg_alert" style="display: none; color: red;">Такой логин уже существует, или вы ввели не верно повторный пароль</p>
				<?php /* Авторизация */ ?>
				<?php /* * надпись, высвечивающаяся при ошибки в отправки формы авторизации */ ?>
				...
				<label class="label-form" for="signin">Войти</label> <?php /* * вкладка авторизации */ ?>

				<input class="signup-input" id="signup" name="action" type="radio" value="signup">
				<label class="label-form" for="signup">Регистрация</label> <?php /* * вкладка регистрации */ ?>

				<div id="wrapper"> <?php /* * блок с полями ввода */ ?>
					<div id="arrow"></div> <?php /* * значок, указывающий на открытую вкладку */ ?>
				...
				<input type="checkbox" id="cbx" style="display: none;"> <?php /* * чекбокс принятия "Пользовательского соглашения" */ ?>
				<?php /* * блок с ссылкой на открытие окна с "Пользовательским соглашением" */ ?>
				<label for="cbx" class="check  check2" style="display:none;">
					<span class="disable_block444as">Cогласен с политикой</span>
					<svg width="25px" height="25px" viewBox="0 0 18 18"> <?php /* * значок галочки в чекбоксе */ ?>
...
<?php if ( isset ($_SESSION['logged_user']) ) : ?> <?php /* * добавление меню пользователя для авторизированного пользователя */ ?>
	<div class="showbackground">
		<nav class="nav">
			...
			<div class="lk" id="block">
				<?php /* * форма для загрузки аватарки пользователя */ ?>
				<form action="avatar.php" method="post" enctype="multipart/form-data">
					<label for="pct" class="lla" style="background-image: url('<?php $id = $_SESSION['logged_user']->id; $cat = R::load('users', $id); echo $cat->filename; ?>'); background-size: cover;background-repeat: no-repeat;"></label> <?php /* * аватарка пользователя */ ?>
					<input type="file" name="filename" id="pct" onchange="this.form.submit ()"><br>
				</form>
				<?php /* * блок со "стоимостью", местом в рейтинге и ссылкой на сообщения с сообществом VK для пополнения баланса */ ?>
				<div class="lk-info">
					<h3>
...
?>
			<?php /* * форма заполнения данных пользователя (номен, ссылка на страницу VK, ссылка на Инстаграм и информация "О себе") */ ?>
			<form action="index.php" method="POST" style="margin-top: 54px;" id="form_profile">
				...
				<button class="paymetod" style="margin-top: 40px;" name="do_account">Сохранить</button> <?php /* * отправка формы */ ?>
			</form>

			<a href="/logout.php">Выйти</a> <?php /* * кнопкой с выходом из аккаунта */ ?>
		</nav>
	</div>
...
<?php if ( isset ($_SESSION['logged_user']) ) : ?> <?php /* * для авторизированного пользователя */ ?>
	<script>
		// * в каждой строчке в таблице на месте рейтинга поставить число
		$('.carousel-group').each(function() {
			$('.table-header').each(function(i) {
...
<?php else : ?> <?php /* * для неавторизированного пользователя */ ?>
	<script>
		// * в каждой строчке в таблице на месте рейтинга поставить число
		$('.carousel-group').each(function(){
			$('.table-header').each(function(i){
...
<link rel="stylesheet" href="css/styles_index_php.css"> <?php /* * стили, перенесённые отсюда в отдельный файл */ ?>
<script type="text/javascript" src="js/script_index_php.js"></script> <?php /* * скрипты, перенесённые отсюда в отдельный файл */ ?>
<style>
	.flex-container {
